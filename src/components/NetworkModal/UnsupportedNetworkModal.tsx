...
export function UnsupportedNetworkModal({ pageSupportedChains }: { pageSupportedChains: number[] }) {
  ...
  const chainId = useLocalNetworkChain() || ChainId.PULSE_CHAIN
  ...
  return (
    <Modal title={t('Check your network')} hideCloseButton headerBackground="gradientCardHeader">
      <Grid style={{ gap: '16px' }} maxWidth="336px">
        <Text>
          {t('Currently %feature% only supported in', { feature: typeof title === 'string' ? title : 'this page' })}{' Pulse Chain'}
          ...
          <Button
            ...
            onClick={() => {
              if (supportedMainnetChains.map((c) => c.id).includes(chainId)) {
                ...
                switchNetworkAsync(ChainId.PULSE_CHAIN)
              }
            }}
          >
        ...
        {isConnected && (
          <Button
            ...
            onClick={() =>
              logout().then(() => {
                switchNetworkLocal(ChainId.PULSE_CHAIN)
              })
            }
          >
            {t('Disconnect Wallet')}
